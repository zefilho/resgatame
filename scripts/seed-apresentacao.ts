...
async function seedApresentacaoFromCSV() {
  const csvPath = path.resolve(process.cwd(), 'scripts', 'lojinha-apresentacao.csv');
  ...
  // Ignorar o cabeçalho (descrição;preço)
  const dataLines = lines.slice(1);
  console.log(`🚀 Carregados ${dataLines.length} itens para a categoria "Lojinha - Apresentação".\n`);
    ...
    if (parts.length >= 2) {
      ...
      if (productName) {
        const menuItemDoc = {
          name: productName,
          price: priceNumber,
          category: 'Lojinha - Apresentação',
        };

        await addDoc(menuRef, menuItemDoc);
        insertedCount++;
        console.log(`   + [Lojinha - Apresentação ${insertedCount}] ${menuItemDoc.name} - R$ ${menuItemDoc.price.toFixed(2)}`);
      }
    }
  }

  console.log(`\n✅ Concluído! Total de ${insertedCount} produtos da Lojinha - Apresentação cadastrados com sucesso no Firestore.`);
  process.exit(0);
}

seedApresentacaoFromCSV().catch((err) => {
  console.error('❌ Erro durante o povoamento da Lojinha - Apresentação:', err);
  process.exit(1);
});
