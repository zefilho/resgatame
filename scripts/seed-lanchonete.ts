...
async function seedLanchoneteFromCSV() {
  const csvPath = path.resolve(process.cwd(), 'scripts', 'cardapio-lanchonete.csv');
  ...
  console.log(`🚀 Carregados ${dataLines.length} itens do cardápio para a categoria "Lanchonete".\n`);
  ...
  for (const line of dataLines) {
    ...
    if (parts.length >= 2) {
      ...
      if (productName) {
        ...
        insertedCount++;
        console.log(`   + [Lanchonete ${insertedCount}] ${menuItemDoc.name} - R$ ${menuItemDoc.price.toFixed(2)}`);
      }
    }
  }

  console.log(`\n✅ Concluído! Total de ${insertedCount} produtos da Lanchonete cadastrados com sucesso no Firestore.`);
  process.exit(0);
}

seedLanchoneteFromCSV().catch((err) => {
  console.error('❌ Erro durante o povoamento do cardápio da Lanchonete:', err);
  process.exit(1);
});
