...
async function seedSantosAnjosFromCSV() {
  const csvPath = path.resolve(process.cwd(), 'scripts', 'lojinha-santos-anjos.csv');
  ...
  console.log(`🚀 Carregados ${dataLines.length} itens para a categoria "Lojinha - Santos Anjos".\n`);
    ...
    if (parts.length >= 2) {
      ...
      if (productName) {
        const menuItemDoc = {
          name: productName,
          price: priceNumber,
          category: 'Lojinha - Santos Anjos',
        };

        await addDoc(menuRef, menuItemDoc);
        insertedCount++;
        console.log(`   + [Lojinha - Santos Anjos ${insertedCount}] ${menuItemDoc.name} - R$ ${menuItemDoc.price.toFixed(2)}`);
      }
    }
  }

  console.log(`\n✅ Concluído! Total de ${insertedCount} produtos da Lojinha - Santos Anjos cadastrados com sucesso no Firestore.`);
  process.exit(0);
}

seedSantosAnjosFromCSV().catch((err) => {
  console.error('❌ Erro durante o povoamento da Lojinha - Santos Anjos:', err);
  process.exit(1);
});
