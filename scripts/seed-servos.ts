...
async function seedServosFromCSV() {
  const csvPath = path.resolve(process.cwd(), 'scripts', 'lista-servos.csv');
  ...
  console.log(`🚀 Carregados ${dataLines.length} registros para cadastro com TAG "Servo".\n`);
  ...
  for (const line of dataLines) {
    ...
    if (parts.length >= 2) {
      ...
      if (rawName) {
        ...
        await addDoc(customersRef, servoDoc);
        insertedCount++;
        console.log(`   + [Servo ${insertedCount}] ${servoDoc.name} (${servoDoc.phone})`);
      }
    }
  }

  console.log(`\n✅ Concluído! Total de ${insertedCount} Servos cadastrados com sucesso no banco de dados.`);
  process.exit(0);
}

seedServosFromCSV().catch((err) => {
  console.error('❌ Erro durante o povoamento de Servos:', err);
  process.exit(1);
});
